...
with tab1:
	...
	if option == "Paraguai":
		...
		with coluna9:
	            bx = px.bar(topvalor, x="pais", y="valor", color_discrete_sequence=['#c43366'], width=800, height=500, range_y=[0, 41_000_000], title="Top 5 Países - Valor Exportado (US$) - Período: 2008 a 2022")
	            bx.update_xaxes(dtick=1)
	            bx.update_layout(
	            yaxis=dict(
	            title="Valor Exportado (US$)",
