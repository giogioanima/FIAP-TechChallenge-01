...
with tab1:
    ...
    with coluna5:
        st.write("Em 2010, as exportações de vinho brasileiro enfrentaram dificuldades devido ao câmbio desfavorável e à crise econômica internacional iniciada em 2008. Os preços no mercado internacional caíram, tornando a competitividade da uva brasileira mais desafiadora. Além disso, novas ameaças surgiram, como a produção de uvas sem sementes em escala empresarial no Peru, onde os custos são menores.")
	st.write("Em 2013, as exportações de vinho tiveram um pico significativo devido ao aumento expressivo no volume exportado, impulsionado pela exportação do vinho a granel, tanto de mesa quanto fino, aproveitando o programa de escoamento de produção (PEP), que conferia vantagem competitiva no mercado internacional. No entanto, houve uma desvalorização no valor do produto nesse ano. Já em 2015, as exportações sofreram uma queda abrupta devido à não adoção do PEP, resultando em uma diminuição de 1171% no volume exportado em relação a 2013. Além disso, a valorização do produto não foi suficiente para compensar a ausência do programa. O contexto de crise política e de corrupção na Petrobras também contribuiu para o cenário desfavorável, impactando negativamente a economia brasileira como um todo.")
	st.write("Desde 2020, a exportação de vinhos tem crescido consistentemente, impulsionada pela pandemia. Com as restrições de viagens impostas pela pandemia, muitas pessoas passaram a consumir mais produtos em suas residências, incluindo o vinho, em vez de frequentar restaurantes, bares ou realizar viagens que envolvessem atividades sociais. Essa mudança de comportamento resultou em uma diminuição do consumo doméstico em alguns países, enquanto a demanda por vinho aumentou internacionalmente. Essa tendência se consolidou em 2021, com recordes consecutivos nas exportações brasileiras de vinhos e espumantes, evidenciando a contínua busca por produtos de qualidade para desfrutar em casa durante o período de isolamento.")
